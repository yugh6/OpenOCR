...
class RotateRatioDataSetTVResize(Dataset):
    ...
    def __init__(self, config, mode, logger, seed=None, epoch=1, task='rec'):
        ...
        # ======= 核心优化 ：预先 O(1) 分桶 =======
        self.ratio_bucket = {}
        unique_ratios = np.unique(self.wh_ratio)
        for r in unique_ratios:
            self.ratio_bucket[r] = np.where(self.wh_ratio == r)[0].tolist()
        self.logger.info("Ratio buckets pre-computed successfully!")
    ...
    def __getitem__(self, properties):
        ...
        if sample_info is None:
            ...
            ratio_ids = self.ratio_bucket.get(ratio, [])
            # ids = random.sample(ratio_ids, 1)
            ids = random.choice(ratio_ids)
            return self.__getitem__([img_width, img_height, ids, ratio])
