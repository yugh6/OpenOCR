...
def rec_collate_fn(batch):
    ...
    for item in batch:
        ...
        label = item[1]
        label = np.pad(label, (0, max_len - len(label)), constant_values=0)
        labels.append(label)
        lengths.append(item[2])
        real_ratios.append(item[3])
    images = torch.stack(images)
    labels = torch.tensor(np.array(labels), dtype=torch.long)
    lengths = torch.tensor(lengths, dtype=torch.long)
    real_ratios = torch.tensor(real_ratios, dtype=torch.float)
    return images, labels, lengths, real_ratios
