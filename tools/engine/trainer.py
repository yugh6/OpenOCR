...
class Trainer(object):
    ...
    def label(self, output_path):
        ...
        env_1 = lmdb.open(output_path, map_size=1099511627776)
        env_2 = lmdb.open(output_path + '_low', map_size=1099511627776)
        cache_1 = {}
        cache_2 = {}
        cnt_1 = 1
        cnt_2 = 1
        ...
        self.model.eval()
        with torch.no_grad():
            for batch in tqdm(self.valid_dataloader, desc="Inference"):
                images = batch[0].to(self.device)
                imgs = batch[3]

                if self.scaler:
                    with torch.cuda.amp.autocast(enabled=self.device.type == 'cuda'):
                        preds = self.model(images, data=None)
                else:
                    preds = self.model(images, data=None)

                preds = self.post_process_class(preds, None)  # list of (pred, score)

                for i in range(len(images)):
                    img = imgs[i]
                    pred, score = preds[i]
                    w, h = Image.open(io.BytesIO(img)).convert('RGB').size
                    if score >=0.90:
                        image_key  = 'image-%09d'.encode() % cnt_1
                        label_key  = 'label-%09d'.encode() % cnt_1
                        score_key  = 'score-%09d'.encode() % cnt_1
                        wh_key     = 'wh-%09d'.encode() % cnt_1

                        cache_1[image_key] = img
                        cache_1[label_key] = str(pred).encode()
                        cache_1[score_key] = str(score).encode()
                        cache_1[wh_key] = (str(w) + '_' + str(h)).encode()

                        cnt_1 += 1

                        if cnt_1 % 1000 == 0:
                            with env_1.begin(write=True) as txn:
                                for k, v in cache_1.items():
                                    txn.put(k, v)
                            cache_1 = {}
                    else:
                        image_key  = 'image-%09d'.encode() % cnt_2
                        label_key  = 'label-%09d'.encode() % cnt_2
                        score_key  = 'score-%09d'.encode() % cnt_2
                        wh_key     = 'wh-%09d'.encode() % cnt_2

                        cache_2[image_key] = img
                        cache_2[label_key] = str(pred).encode()
                        cache_2[score_key] = str(score).encode()
                        cache_2[wh_key] = (str(w) + '_' + str(h)).encode()
                        cnt_2 += 1

                        if cnt_2 % 1000 == 0:
                            with env_2.begin(write=True) as txn:
                                for k, v in cache_2.items():
                                    txn.put(k, v)
                            cache_2 = {}
            ...
            # 写入样本数
            with env_1.begin(write=True) as txn:
                txn.put('num-samples'.encode(), str(cnt_1-1).encode())
            with env_2.begin(write=True) as txn:
                txn.put('num-samples'.encode(), str(cnt_2-1).encode())

        env_1.close()
        env_2.close()
        print(f"Saved {cnt_1-1} samples to {output_path}")
        print(f"Saved {cnt_2-1} samples to {output_path}_low")
